...
public class DataProviderJdbc implements DataProvider {
    ...
    public boolean appendOrUpdateSecurityHistory(SecurityHistory securityHistory, String ticker){
        log.info("Starting DataProviderJdbc appendOrUpdateSecurityHistory[0]");
        try {
            ...
            if( connection.createStatement().executeUpdate(sqlUpdateSecurityHistory(securityHistory, ticker)) != 1) {
                log.debug("appendOrUpdateSecurityHistory[4]: Appending security history");
                connection.createStatement().executeUpdate(String.format(SQL_INSERT, ticker)
            ...
            log.error("Function DataProviderJdbc appendOrUpdateSecurityHistory had failed[5]: {}", e.getMessage());
        }
        return false;
    }
        ...
        try {
            ...
            for (User user : userList)
                if( connection.createStatement().executeUpdate(sqlUpdateUser(user)) == 0)
                    ...
                    MongoHistory.save(UPDATE, JDBC, user);
            if (response.isEmpty())
    ...
    public Optional<User> updateUser(String id, String name){
        ...
        try {
            ...
            log.error("Function DataProviderJdbc updateUser had failed[4]: {}", e.getMessage());
        }
        return Optional.empty();
    }
    ...
    public Optional<User> deleteUserById(String id) {
        ...
        try {
            ...
            if (rs.next()){
                ...
                MongoHistory.save(UPDATE, JDBC, resultSetToUser(rs));
                return Optional.of(resultSetToUser(rs));
            }
        }catch (Exception e){
    ...
    public Result<User> deleteAllUsers(){
        ...
        try {
            ...
            deleteAllActions();
            MongoHistory.save(UPDATE, JDBC, result.getBody());
    ...
    public Result<Action> deleteActionHistory(String userId){
        ...
        try {
            ...
            connection.createStatement().executeUpdate(String.format(SQL_DELETE_FROM, ACTON_TABLE_NAME)
                    .concat(SQL_WHERE).concat(String.format(SQL_ACTION_USER_ID, userId)));
            MongoHistory.save(UPDATE, JDBC, actions);
    ...
    public String showHistory(String userId){
        log.info("Starting DataProviderJdbc showHistory[0]");
        try {
            ...
            log.error("Function DataProviderJdbc showHistory had failed[3]: {}" , e.getMessage());
        }
        return "";
    }
